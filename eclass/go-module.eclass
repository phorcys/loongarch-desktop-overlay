...
post_src_unpack() {
    if use loong; then
        elog "======= Go on LoongArch : ===="
        elog "     golang.org/x/sys package is not upstreamed yet. replace with github.com/loongson/sys ."
        #echo " replace golang.org/x/sys => github.com/loongson/sys master">> go.mod
        if [ -d vendor/golang.org/x/sys ]; then
            rm -rf vendor/golang.org/x/sys vendor/golang.org/x/net
            cp -rf /usr/lib/go/src/vendor/golang.org/x/sys vendor/golang.org/x/
			cp -rf /usr/lib/go/src/vendor/golang.org/x/net vendor/golang.org/x/
        fi
    fi
